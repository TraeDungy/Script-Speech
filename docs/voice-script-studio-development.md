...
- Persist contextual metadata (genre, format, characters, locations, props, tone, constraints) across the project lifecycle.
...
- Automated backups for Postgres; object storage lifecycle policies (30-day retention default).
...
| M2 | Outline → scene generation, script preview | 2–3 weeks |
...
6. Export pipeline delivers valid TXT, PDF, Fountain; DOCX and FDX available (feature flag if necessary).
7. Email delivery succeeds with downloadable artifacts.
8. `make dev` command boots full local stack with mock keys and passes health checks.
