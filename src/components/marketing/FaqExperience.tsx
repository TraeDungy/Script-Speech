...
export function FaqExperience({ faq, showFallbackNotice = false }: FaqExperienceProps) {
  return (
    <main className="mx-auto flex min-h-screen max-w-5xl flex-col gap-20 px-6 py-16 md:px-10">
        ...
        {faq.coreFeatures?.length ? (
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {Array.from({ length: 4 }).map((_, index) => (
              <div key={`core-feature-skeleton-${index}`} className="rounded-3xl border border-white/10 bg-white/5 p-6">
        ...
        {faq.workflowStages?.length ? (
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {Array.from({ length: 3 }).map((_, index) => (
              <div key={`workflow-skeleton-${index}`} className="rounded-3xl border border-white/10 bg-white/5 p-6">
      ...
      <section className="space-y-10">
        ...
        {faq.platformPillars?.length ? (
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {Array.from({ length: 3 }).map((_, index) => (
              <div key={`pillar-skeleton-${index}`} className="rounded-3xl border border-white/10 bg-white/5 p-6">
